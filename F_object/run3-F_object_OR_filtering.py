...
legend_or_tennis = nr.filter(F(legend=True) | F(location='court')) #filtering devices with the legends=True including using the OR (|) logic to filter.
results = legend_or_tennis.run(task=banner_push) #pushing banner_push to specified devices that is mapped to the legends and courts keys in groups.yaml.  Also mapped on hosts.yaml file.
