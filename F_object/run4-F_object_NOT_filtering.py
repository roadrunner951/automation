...
not_tennis = nr.filter(~F(location='court')) #filtering devices using the NOT (~) logic.
results = not_tennis.run(task=banner_push) #pushing banner_push to specified devices that is mapped to the legends and courts keys in groups.yaml.  Also mapped on hosts.yaml file.
